...
for outer = 1:length(eps)
    ...
    for j=1:10
        ...
        chisq_cut(j) = perm_cutoff(X,Y,fchisq);
        ellone_cut(j) = perm_cutoff(X,Y,fellone);
        elltwo_cut(j) = perm_cutoff(X,Y,felltwo);
    end
    chisq_cut = mean(chisq_cut);
    ellone_cut = mean(ellone_cut);
    elltwo_cut = mean(elltwo_cut);
...
labels{6} = 'Oracle GoF';
labels{7} = 'L2';
plot_sim(data,labels,'Plots/two_sample_power_imbalance.eps',N);
