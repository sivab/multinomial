...
for i = 1:length(alt_distance)
    ...
    alt_dist = generate_alternate_distribution(1,null_dist,alt_distance(i));
...
title('Power Law Null, Uniform Alternate','Interpreter','latex');
xlim([0 max(x)]);
ylim([0 1]);
xlabel('$$\ell_1$$ Distance','Interpreter','latex');
ylabel('Power','Interpreter','latex');
set(gcf, 'PaperPositionMode', 'auto');
filename = sprintf('Plots/n%d_d%d_power_unif_full.eps',n,d);
